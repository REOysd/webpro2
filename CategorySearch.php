...
function fetchCategoryName($conn, $cid) {
    $cid = (int)$cid;
    $res = mysqli_query($conn, "SELECT CategoryName FROM GoodsCategory WHERE CategoryID = {$cid} LIMIT 1");
    if (!$res) return null;
    $row = mysqli_fetch_assoc($res);
    mysqli_free_result($res);
    return $row ? $row['CategoryName'] : null;
}

function fetchGoods($conn, $cid, $q) {
    $where = ["g.Stock > 0"];
    $types = '';
    $params = [];
    $hasFilter = false;
...
if (!$conn) {
    ...
    if ($cid !== '') {
        $categoryName = fetchCategoryName($conn, $cid);
    }
    list($goodsRows, $err) = fetchGoods($conn, $cid, $q);
    if ($err) $error = $err;
    mysqli_close($conn);
}
...
function buildDetailUrl($gid, $cid, $q) {
    $url = 'GoodDisplay.php?gid=' . rawurlencode((string)$gid);
    $parts = [];
    if ($cid !== '') $parts[] = 'cid=' . rawurlencode((string)$cid);
    if ($q !== '') $parts[] = 'q=' . rawurlencode((string)$q);
    if (count($parts) > 0) {
        $url .= '&' . implode('&', $parts);
    }
    return $url;
    ...
    <div class="wrap">
        ...
        <?php } else { ?>
            <div class="search-info">
                <?php if ($categoryName) { ?>
                    <p>カテゴリ「<?php echo htmlspecialchars($categoryName, ENT_QUOTES, 'UTF-8'); ?>」で検索</p>
                <?php } ?>
                <?php if ($q !== '') { ?>
                    <p>商品名「<?php echo htmlspecialchars($q, ENT_QUOTES, 'UTF-8'); ?>」で検索</p>
                <?php } ?>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><a class="detail-btn" href="<?php echo htmlspecialchars(buildDetailUrl($row['GoodsID'], $cid, $q), ENT_QUOTES, 'UTF-8'); ?>">詳細</a></td>
                        </tr>
                        <?php } ?>
                    </tbody>
                </table>
